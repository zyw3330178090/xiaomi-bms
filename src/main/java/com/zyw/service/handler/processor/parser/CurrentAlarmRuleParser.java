 ...
 * 电流信号解析
 */
public class CurrentAlarmRuleParser {
     ...
     * 解析预警规则字符串
     */
    public CurrentAlarmRuleParser(String ruleStr) {
        this.ruleStr = ruleStr;
        // 按逗号分割各个规则
        String[] ruleItems = ruleStr.split(",");

        // 正则表达式匹配规则(修改了关键字为current)
        Pattern pattern = Pattern.compile("(\\d*\\.?\\d+)?<=?current<?(\\d*\\.?\\d+)?:(\\-?\\d+)");
    ...
    public int calculateAlarmLevel(BatterySignal signal) {
        double currentDiff = signal.getIx() - signal.getIi();
        if (ruleStr.startsWith("3")) {
            if (currentDiff >= 3) {
                return 0;
            } else if (currentDiff >= 1) {
                return 1;
            } else if (currentDiff >= 0.2) {
                return 2;
            }
        } else if (ruleStr.startsWith("1")) {
            if (currentDiff >= 1) {
                return 0;
